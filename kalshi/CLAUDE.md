...
| **Connection Manager** | Maintains 150 WebSocket connections per gatherer (144 orderbook + 6 global). Uses batch subscriptions (1000 markets/command) for initial sync. |
...
- **Batch subscriptions** - Subscribe to 1000 markets per WebSocket command (reduces 600K commands to 600)
...
- **Batch subscriptions**: Use `market_tickers` array (up to 1000 markets) instead of single `market_ticker` for initial sync
- **Sequence tracking**: Key by `(connID, SID)` tuple, not just SID (SIDs are per-connection, not global)
- **Config**: `SubscribeBatchSize` in ManagerConfig (default: 1000)

### API Client

- `StreamMarketsWithOptions()`: Callback-based pagination for processing markets as pages are fetched
- `GetAllMarketsWithOptions()`: Returns all markets after fetching all pages (blocking)
