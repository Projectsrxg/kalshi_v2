...
When an orderbook connection fails, markets are **immediately redistributed** to healthy connections:

1. Markets are moved from failed connection to healthy ones (load-balanced)
2. Failed connection attempts reconnect in background
3. On reconnect, connection is available for new markets (old ones stay redistributed)

This minimizes data gaps compared to waiting for reconnection.
