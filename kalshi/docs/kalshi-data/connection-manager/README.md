...
| `orderbook_delta` | Required | Batch subscription, least-loaded assignment |

### Batch Subscriptions

For `orderbook_delta`, markets are subscribed in batches to reduce the number of WebSocket commands during initial sync:

- **Batch size:** 1000 markets per subscribe command (configurable via `SubscribeBatchSize`)
- **Assignment:** Markets are assigned to connections using least-loaded balancing
- **Command format:** Uses `market_tickers` array instead of single `market_ticker`
...
```json
{
  ...
  "params": {
    ...
    "market_tickers": ["MARKET-1", "MARKET-2", ..., "MARKET-1000"]
  }
}
```

With 600K markets across 144 connections:
- **Without batching:** 600K individual subscribe commands
- **With batching (1000/batch):** ~600 subscribe commands (1000x reduction)

Live market updates (new markets, status changes) still use individual subscriptions for simplicity.
