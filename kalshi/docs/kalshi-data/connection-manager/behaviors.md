...
**Important:** Sequence numbers are tracked per `(connID, SID)` tuple, not just SID. SIDs are unique per-connection, not globally - multiple connections can have the same SID for their subscriptions.
